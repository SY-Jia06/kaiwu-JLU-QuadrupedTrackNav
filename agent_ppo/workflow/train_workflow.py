...
def report_monitor_data(ep_infos, reward_keys, agent, monitor, episode, storage_stats=None):
    ...
    if storage_stats:
        ...
        for key in (
            ...
            if key in storage_stats:
                monitor_data[key] = storage_stats[key]
...
def _compute_advantages_and_returns(storage, agent, critic_obs, logger):
    ...
    if storage.observations.shape[-1] >= 9:
        obs = storage.observations
        storage_stats["nav_cmd_vx_mean"] = obs[..., 6].mean().item()
        storage_stats["nav_cmd_vy_abs_mean"] = torch.abs(obs[..., 7]).mean().item()
        storage_stats["nav_cmd_wz_abs_mean"] = torch.abs(obs[..., 8]).mean().item()

        if obs.shape[-1] >= 13:
            storage_stats["nav_goal_dist_mean"] = (obs[..., -2] * 20.0).mean().item()
            storage_stats["nav_target_yaw_abs_mean"] = (torch.abs(obs[..., -1]) * torch.pi).mean().item()
